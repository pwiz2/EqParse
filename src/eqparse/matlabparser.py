...
class MatlabParser (BaseParse):
	...
	def __init__ (self, Lib):
		super(MatlabParser, self).__init__()
		self.l_enclose = "("
		self.r_enclose = ")"
		self.operations = ['\nif(', ")\n", "else\n\t", "elif\n", "end\n", "~=", "(", ").^(", ")","sqrt("]
	...
	def multi_runfile (self, newSubDirectory = "", 
		...
		# start ode
		mwrite (mfio, ["options = odeset('InitialStep',0.02,'MaxStep',25, 'RelTol', 1e-9, 'AbsTol', 1e-9);","[t," + m_ode + "] = " + ode_method + "(@(tt,y)" + m_ode_fname + "(tt,y," 
								+ m_noprint + "," + m_print + ")," +
		...
		num_iter = [list(filter(str.isdigit, elem)) for elem in lhs]		# get number from derived list
	...
	def supp_initialise_data (self):
		...
		# initialise function
		mwrite (mfio, "function [" + m_print + "," + m_noprint + "," + m_ode + "] = " + mfile + "(" + m_initial_param + "," + m_noprint + "_in," + m_ode + "_in)\n\n")
		...
		mwrite (mfio, "if nargin==1")
		mcomment (mfio, "preallocate storage for data as contigent memory")
		mwrite (mfio, [m_noprint + " = zeros(1," + str(len(mI_user_no_ode)) + ");",
		...
		mwrite (mfio, "if nargout==1")	
		mwrite (mfio, "TEMPSTRUCT = struct;")	
		self.pattern_write (mfio, [self.names, "name", "comment"], [ "TEMPSTRUCT.", " = ", "; % ", ""], self.data_vec)
		mwrite (mfio, m_print + " = TEMPSTRUCT;")
		mwrite (mfio, "end")
		
		
		######################
		
		# gather back initial data struct
		mwrite (mfio, "elseif nargin==3")	
			
		mIndex_p = self.new_index(self.index['print']['no'],self.index['ode']['no'])
		mIndex_pstore = self.new_index(self.index['print']['yes'],self.index['ode']['no'])
		mIndex_ode = self.index['ode']['yes']
	...
	def FSA_cvodes (self):
		...
		mwrite (mfio, "function [" + m_ode + ", flag, new_data] = " + f_ode + "(t," + m_ode_rhs + ", " + m_data + ")\n\n")
											
		mIndex_static =  self.order_index(self.new_index(self.index['rhs']['yes'],self.index['ode']['no'])) 
		m_lhs = self.get_list('name', self.data_vec)
		
		oderhs_names = self.vectorise_name (self.index['ode']['yes'], "y", m_lhs)
		...
		index_nonode = self.new_dependancy_index(self.new_index(self.index['ode']['no'],self.index['rhs']['yes']),'rhs')
		self.pattern_write (mfio, [m_lhs, m_rhs, "comment"], [ "\t", " = ", "; %", ""], self.data_vec, index_nonode)
			# print all ODE related dependencies out to file in last order
		self.pattern_write (mfio, [m_lhs, m_rhs, "comment"], [ "\t", " = ", "; %", ""], self.data_vec, self.index['ode']['yes'])			
		...
		# preallocate storage
		#--#-- now only user-defined, the rest is explicitly defined
		mI_user_param = self.new_index(self.index['userdef']['yes'],self.index['ode']['no'])
		mI_user_no_ode = self.new_index(self.index['print']['no'],self.index['ode']['no'])
		mI_uder_no_ode_but_print = self.new_index(self.index['print']['yes'],self.index['ode']['no'])
		mcomment (mfio, "preallocate storage for data")
		...
		mwrite (mfio, "CVodeSensInit(Ns, [], " + m_outsens + ", FSAoptions);\n")
		...
		mwrite (mfio, "end % end of if loop \n\n","\n")
	...
	def FSA_ode (self):
		...
		mIndex_static =  self.order_index(self.new_index(self.index['rhs']['yes'],self.index['ode']['no'])) 
		m_lhs = self.get_list('name', self.data_vec)
		for i in mIndex_static:
			m_lhs[i] = self.names[i]
